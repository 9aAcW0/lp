...
func SearchUserDataByMail(mail string) ([]string, int, *sheets.SheetClient) {
	formData, client := sheets.GetAll()
	rowNum := 0
	for _, d := range formData {
		if d[1] == mail {
			user := convertInterfaceToString(d)
			return user, rowNum, client
		}
		rowNum += 1
	}
	return nil, 0, nil
...
func SearchUserDataByCorporateID(corporateID string) ([][]string, *sheets.SheetClient){
	formData, client := sheets.GetAll()
	var users [][]string
	for _, d := range formData {
		if d[2] == corporateID {
