...
func GetCarrierUpData(userdata []string, rowNum int, client *sheets.SheetClient) models.CarrierUp {
	answer := userdata[11:26]
	// はいで加点を整形
	yesAdd := answer[4:13]
	yesAdd = append(yesAdd, answer[2])
	// いいえで加点を整形
	noAdd := answer[0:2]
	noAdd = append(noAdd,answer[3])
	...
	if client != nil {
		CheckRateCategory(client, answerInt, rowNum, "carrierUp")
	}
	return CarrierUp
}
