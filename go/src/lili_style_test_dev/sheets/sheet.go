...
func (s *SheetClient) Update(rate int,column string,row int) error {
	valueRange := "Answer!" + column + strconv.Itoa(row)
	...
	resp, err := s.srv.Spreadsheets.Values.Update(s.spreadsheetID, valueRange, rb).ValueInputOption("USER_ENTERED").Do()
	if err != nil {
		return err
	}
	if resp == nil {
...
func GetAll() ([][]interface{}, *SheetClient){
...
func GetCorporateSheet() ([][]interface{}, *SheetClient){
	ctx := context.Background()
	client, err := NewSheetClient(ctx, "1LcGnzRuKYA5SyOBGsUm1oN1r4pNX-8lmaTYT54A_RsI")
	if err != nil {
		fmt.Println(err)
